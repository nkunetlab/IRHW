...
parser.add_argument('--outf', default='./heritage_model_distilltion_model/', help='folder to output images and model checkpoints')
...
trainset = torchvision.datasets.ImageFolder(root='../heritage_watermark_heritage_2/train', transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=BATCH_SIZE, shuffle=True, num_workers=2)

testset = torchvision.datasets.ImageFolder(root='../heritage_watermark_heritage_2/test', transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False, num_workers=2)

# Model definition
teacher_net = torchvision.models.resnet50(pretrained=False).to(device)
teacher_net.fc = nn.Linear(512 * 4, 10).to(device)
teacher_net.load_state_dict(torch.load('./heritage_key_model/net_128.pth'))

student_net = torchvision.models.googlenet(pretrained=False).to(device)
student_net.fc = nn.Linear(1024, 10).to(device)
...
if __name__ == "__main__":
    ...
    with open("acc_heritage_model_distilltion.txt", "w") as f:
        with open("log_heritage_model_distilltion.txt", "w")as f2:
            teacher_net.eval()
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    # record test acc
                    if acc > best_acc:
                        f3 = open("best_acc_heritage_model_distilltion.txt", "w")
