...
class IRNModel(BaseModel):
    def __init__(self, opt):
        ...
        if not self.is_train:
            if opt['upsample']:
                self.other_upsample = torch.load(opt['upsample'])
                self.other_upsample = self.other_upsample.to(self.device)
            else:
                self.other_upsample = None
    ...
    def test(self):
        ...
        self.netG.eval()
        with torch.no_grad():
            first_output = self.netG(x=self.input)
            self.forw_L = first_output[:, :3, :, :]
            self.upsample = first_output[:, 3:, :, :]
            self.forw_L = self.Quantization(self.forw_L)
            curr_upsample = gaussian_scale * self.gaussian_batch(zshape)
            if not self.other_upsample is None:
                curr_upsample[0, :, :, :] = self.other_upsample
            y_forw = torch.cat((self.forw_L, curr_upsample), dim=1)
    ...
    def get_current_visuals(self):
        ...
        out_dict['upsample'] = self.upsample.detach()[0].float().cpu()
        return out_dict
