...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        upsample = visuals['upsample']

        diff_sr_mean = 255 - cv2.absdiff(util.tensor2img(visuals['GT']), util.tensor2img(visuals['SR']))
        # diff_sr_mean = np.reshape(diff_sr_mean, (diff_sr_mean.shape[0], diff_sr_mean.shape[1], -1))
        ...
        if suffix:
            save_img_path = osp.join(dataset_dir, img_name + suffix + '_upsample.pth')
        else:
            save_img_path = osp.join(dataset_dir, img_name + '_upsample.pth')
        torch.save(upsample.clone(), save_img_path)

        if suffix:
            save_img_path = osp.join(dataset_dir, img_name + suffix + '_diff_SR_mean.png')
        else:
            save_img_path = osp.join(dataset_dir, img_name + '_diff_SR_mean.png')
        util.save_img(diff_sr_mean, save_img_path)
